...
class ServerManager(NetworkManager):
    ...
    def setup(self):
        ...
        - BaseServer accept local client num report from client manager.
...
class SynchronousServerManager(ServerManager):
    ...
    def main_loop(self):
        ...
        BaseServer transmits received package to backend computation handler for aggregation or others
        manipulations.

        Loop:
            1. activate clients for current trainers round.
    ...
    def activate_clients(self):
        ...
        """
        self._LOGGER.info("BaseClient activation procedure")
        clients_this_round = self._handler.sample_clients()
        rank_dict = self.coordinator.map_id_list(clients_this_round)

        self._LOGGER.info("BaseClient id list: {}".format(clients_this_round))
...
class AsynchronousServerManager(ServerManager):
    ...
    def main_loop(self):
        ...
        - BaseServer receive ParameterRequest from client. Send model parameter to client.
        - BaseServer receive ParameterUpdate from client. Transmit parameters to queue waiting for aggregation.
