...
def read_config_from_json(json_file: str, user_name: str):
    """Read configs from `json_file` to get configs for `user_name`

    Args:
        json_file (str): path for json_file
        user_name (str): read configs for this user, it can be 'server' or 'client_id'

    Returns:
        a tuple with ip, port, world_size, rank about user with `user_name`

    Examples:
        read_config_from_json('../../../tests/data/configs.json', 'server')

    Notes:
        configs.json example as follows
        {
          "server": {
            "ip" : "127.0.0.1",
            "port": "3002",
...
def partition_report(targets,
    ...
    Examples:
        ...
        BaseClient   0,0.200,0.00,0.200,0.600,5
        BaseClient   1,0.400,0.200,0.200,0.200,5
        BaseClient   2,0.00,0.400,0.400,0.200,5
    ...
    for client_id in sorted_cid:
        ...
        report_line = f"BaseClient {client_id:3d}," + \
                      ','.join([
